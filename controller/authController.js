...
router.post('/register', (req, res) => {
    ...
    User.create({
        ...
        profilePic:req.body.profilePic?req.body.profilePic:'User'
    },(err,data) => {
...
router.post('/login', (req,res) => {
    User.findOne({email:req.body.email}, (err,user) => {
        ...
        if(!user) return res.send({auth:false,token:'No User Found Register First'})
            ...
            // in case email and password is correct generate token
            let token = jwt.sign({id:user._id},config.secret,{expiresIn:86400})
            return res.send({auth:false,token:token})
        }
    })
})
...
router.get('/userInfo', (req, res) => {
    ...
    jwt.verify(token,config.secret,(err,user) => {
        if(err) return res.send({auth:false,token:'Error Verifying Token'})
